...
Migration `202608030038` enables the blueprint's existing `allow_pause` setting.
The setting is copied into each new session, preserving an already-started
...
Module 4 also needs no schema migration. Its operator import creates volume
`AD_M4`, three ordered chapter/topic rows, governed learning metadata, and 100
draft questions in exact package `AD_M4_100`. Seven distinct `module4_*` visual
objects reuse migration 044's private Storage registry and answer-aware policies.

Module 5 needs no schema migration. Its operator import creates volume `AD_M5`,
four ordered chapter/topic rows, governed learning metadata, and 100 draft
questions in exact package `AD_M5_100`. Seven distinct `module5_*` objects reuse
