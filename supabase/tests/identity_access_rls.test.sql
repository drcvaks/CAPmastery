...
select is((select count(*)::integer from public.profiles), 4, 'Auth trigger creates profiles');
...
select is((select count(*)::integer from public.profiles), 4, 'Admin reads all profiles');
